            ...
            if (item.status){
                ...
                if (!providerName.equals("",true)){
                    if (!providerName.equals("null",true)){
                        binding.tvTextNames.text = "You’ve got a gift from "+BaseApplication.capitalLatter(providerName.toString())
                        binding.tvSecretCookBook.text = BaseApplication.capitalLatter(providerName.toString())+"’s secret cookbook"
                    }
                }
""
