...
class AdapterFilterCuisinesItem(
    private var datalist: MutableList<DishType>,
    private var requireActivity: FragmentActivity,
    private var onItemClickListener: OnItemClickListener
) : RecyclerView.Adapter<AdapterFilterCuisinesItem.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.relMainLayouts.setOnClickListener {
            if (item.name.equals("More")){
                onItemClickListener.itemClick(position, datalist[position].name,"Cuisines")
            }else{
    ...
    fun updateList(newList: MutableList<DishType>) {
        datalist=newList
        notifyDataSetChanged()
    }
