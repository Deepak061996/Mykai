...
class AdapterFilterNutritionItem(
    private var datalist: MutableList<Protein>,
    private var requireActivity: FragmentActivity,
    private var onItemClickListener: OnItemClickListener
) : RecyclerView.Adapter<AdapterFilterNutritionItem.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.relMainLayouts.setOnClickListener {
            if (item.name.equals("More")){
                onItemClickListener.itemClick(position, datalist[position].name,"Nutrition")
            }else{
    ...
    fun updateList(newList: MutableList<Protein>) {
        datalist=newList
        notifyDataSetChanged()
    }
