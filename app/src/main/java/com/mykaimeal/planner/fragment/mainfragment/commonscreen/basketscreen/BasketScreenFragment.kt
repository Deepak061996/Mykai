...
//            viewModelData()
    ...
    private fun initialize() {
        ...
        binding.textConfirmOrder.setOnClickListener {
            if (binding.textConfirmOrder.isClickable) {
                if (binding.textTotalAmount.text.toString().equals("0",true) || binding.textTotalAmount.text.toString().equals("0.0",true)) {
                        showAlert(getString(R.string.available_products), false)
                }else{
                    val count = stores.count { it.is_slected==1 }?:0
                    if (count==0){
                        showAlert(ErrorMessage.selectSuperMarketError, false)
                    }else{
    ...
    private fun showDataInUI(data: BasketScreenModelData) {
        ...
        if (stores.size > 0) {
            stores.sortByDescending { it.is_open }
        ...
        if (ingredientList.size > 0) {
            ...
            if (count.toDouble()==0.0){
                ...
                if (stores.size > 0) {
                    stores.forEach {
                        if (it.is_slected!=null){
                            if (it.is_slected==1){
                                it.total=formattedTotal.toDouble()
                            }
                        }
                    }
                    adapter?.updateList(stores)
