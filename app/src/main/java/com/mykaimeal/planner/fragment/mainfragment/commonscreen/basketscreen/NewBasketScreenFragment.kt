...
import com.mykaimeal.planner.adapter.BasketIngredientsAdapter
...
import com.mykaimeal.planner.databinding.FragmentNewBasketScreenBinding
...
@AndroidEntryPoint
class NewBasketScreenFragment : Fragment(),OnItemLongClickListener , OnItemSelectListener {

    private lateinit var binding: FragmentNewBasketScreenBinding
    private var adapterRecipe: BasketYourRecipeAdapter? = null
    private  var adapterIngredients: BasketIngredientsAdapter? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        binding = FragmentNewBasketScreenBinding.inflate(layoutInflater, container, false)
        recipe.clear()
        ...
        adapterIngredients = BasketIngredientsAdapter(ingredientList, requireActivity(), this)
    ...
    private fun buttonClick(){
        val count = if (ingredientList.isNotEmpty()){
            ingredientList.count {  it.newStatus }
        }else{
            0
        }

        val recipeCount = recipe.size

        if (count==0 || recipeCount==0){
            binding.textConfirmOrder.isEnabled=false
            binding.textConfirmOrder.setBackgroundResource(R.drawable.gray_btn_unselect_background)
        }else{
            binding.textConfirmOrder.isEnabled=true
            binding.textConfirmOrder.setBackgroundResource(R.drawable.gray_btn_select_background)
        }
    ...
    private fun addressDialog() {
        ...
        dialogAddress?.window!!.setLayout(WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.WRAP_CONTENT)
        apiCall="yes"
    ...
    private fun moveToInstaCartBasket(){
        val jsonArray = JsonArray()
        ingredientList.forEach { ingredientsModel ->
            if (ingredientsModel.newStatus) {
                // Create a JsonObject for each ingredient
                val ingredientObject = JsonObject()
                ingredientObject.addProperty("name", ingredientsModel.name)
                ingredientObject.addProperty("quantity", ingredientsModel.quantity.toString())
                ingredientObject.addProperty("unit", ingredientsModel.unit_of_measurement)
        ...
        BaseApplication.showMe(requireContext())
        lifecycleScope.launch {
            basketScreenViewModel.getInstaCart({
                BaseApplication.dismissMe()
                handleInsaCartApiResponse(it)
            }, jsonObject)
        }
    }

    private fun handleInsaCartApiResponse(result: NetworkResult<String>) {
        when (result) {
            is NetworkResult.Success -> handleSuccessInsaCartResponse(result.data.toString())
            is NetworkResult.Error -> showAlert(result.message, false)
            else -> showAlert(result.message, false)
        }
    }
    ...
    @SuppressLint("SetTextI18n")
    private fun handleSuccessInsaCartResponse(data: String) {
        try {
            val apiModel = Gson().fromJson(data, BasketScreenModel::class.java)
            Log.d("@@@ addMea List ", "message :- $data")
            if (apiModel.code == 200 && apiModel.success== true) {
                val bundle = Bundle()
                bundle.putString("linkUrl",apiModel.data?.products_link_url?:"")
                findNavController().navigate(R.id.instacartBasketFragment,bundle)
            } else {
                handleError(apiModel.code,apiModel.message)
            }
        } catch (e: Exception) {
...
//                city = address.locality ?: "" // City
    ...
    private fun showDataInUI(data: BasketScreenModelData) {
        ...
        val count =  recipe.size /*+ ingredientList.size*/
        if (count==0){
            hideView(false)
        }else{
            hideView(true)
    ...
    /*private fun showDataInUI(data: BasketScreenModelData) {
        basketScreenViewModel.setBasketData(data)
        val newRecipes = data.recipe ?: emptyList()
        val newIngredients = data.ingredient?.map { it.copy(newStatus = true) } ?: emptyList()
        recipe.clear()
        recipe.addAll(newRecipes)
        ingredientList.clear()
        ingredientList.addAll(newIngredients)
        binding.rlYourRecipes.visibility = if (recipe.isNotEmpty()) View.VISIBLE else View.GONE
        binding.rlIngredients.visibility = if (ingredientList.isNotEmpty()) View.VISIBLE else View.GONE
        adapterRecipe?.updateList(recipe)
        adapterIngredients?.updateList(ingredientList)
        val count = recipe.size + ingredientList.size
        hideView(count != 0)
    ...
    private fun hideView(status:Boolean){
        if (status){
            ...
            binding.textConfirmOrder.visibility=View.VISIBLE
            binding.layNoBasket.visibility=View.GONE
            binding.textConfirmOrder.isClickable=true
        }else{
            binding.layData.visibility=View.GONE
            binding.textConfirmOrder.visibility=View.GONE
            binding.textConfirmOrder.isClickable=false
            binding.layNoBasket.visibility=View.VISIBLE
        }
    }
    ...
    override fun itemSelect(position: Int?, recipeId: String?, type: String?) {
        if (BaseApplication.isOnline(requireActivity())) {
            ...
            if (type.equals("IngredientsClick",true)){
                ...
                dataChange.newStatus = !dataChange.newStatus
                ingredientList[position] = dataChange
                adapterIngredients?.notifyItemChanged(position)
            }
        }else{
            BaseApplication.alertError(requireContext(), ErrorMessage.networkError, false)
        }
    ...
    private fun handleSuccessIngResponse(
        ...
        try {
            ...
            if (apiModel.code == 200 && apiModel.success) {
                ...
                val count =  /*(recipe.size ?: 0) */+ (ingredientList.size ?: 0)
                if (count==0){
                    hideView(false)
                }else{
                    hideView(true)
