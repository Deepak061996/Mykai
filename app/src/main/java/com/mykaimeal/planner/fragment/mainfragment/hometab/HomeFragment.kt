...
class HomeFragment : Fragment(), View.OnClickListener, OnItemClickListener, OnItemSelectListener {
    ...
    private fun showData(data: com.mykaimeal.planner.fragment.mainfragment.viewmodel.homeviewmodel.apiresponse.DataModel?) {
        try {
            ...
            if (userDataLocal.userData != null && userDataLocal.userData!!.isNotEmpty()) {
            ...
            if (userDataLocal.frezzer != null) {
                ...
                if (userDataLocal.frezzer?.Dessert != null) {
                    binding.layDessert.visibility = View.VISIBLE
                    binding.tvfreezerDessert.text = "" + userDataLocal.frezzer?.Dessert
                    updateCount(userDataLocal.frezzer?.Dessert)
                } else {
                    binding.layDessert.visibility = View.GONE
                }
            }
            ...
            if (userDataLocal.fridge != null) {
                ...
                if (userDataLocal.fridge?.Dessert != null) {
                    binding.layDessert.visibility = View.VISIBLE
                    binding.tvfridgeDessert.text = "" + userDataLocal.fridge?.Dessert
                    updateCount(userDataLocal.fridge?.Dessert)
                } else {
                    binding.layDessert.visibility = View.GONE
                }
            }
            ...
            userDataLocal.monthly_savings?.let {
                if (sessionManagement.getUserName() != null) {
                    val name=BaseApplication.capitalLatter(sessionManagement.getUserName().toString())
                    binding.tvMonthlySavingsDesc.text =
                        "Good job ${name}, you are on track to save $it this month"
                }
            }?:run {
                if (sessionManagement.getUserName() != null) {
                    val name=BaseApplication.capitalLatter(sessionManagement.getUserName().toString())
                    binding.tvMonthlySavingsDesc.text =
                        "Good job ${name}, you are on track to save ${0} this month"
                }
            }
...
//                    getLatLong()
    ...
    private fun initialize() {
        ...
        if (sessionManagement.getUserName() != null) {
            val name = BaseApplication.getColoredSpanned("Hello", "#06C169") + BaseApplication.getColoredSpanned(", " + BaseApplication.capitalLatter(sessionManagement.getUserName().toString()), "#3C4541")
            binding.tvName.text = Html.fromHtml(name)
            binding.tvMonthlySavingsDesc.text="Good job ${BaseApplication.capitalLatter(sessionManagement.getUserName().toString())}, you are on track to save £0 this month"
        }
        ...
        binding.btnTrail.setOnClickListener(this)
...
//        binding!!.relMonthlySavings.setOnClickListener(this)
    ...
    override fun onClick(item: View?) {
        when (item!!.id) {
            R.id.textSeeAll -> {
                (activity as MainActivity?)?.upDateFullCookPlan()
    ...
    override fun itemClick(position: Int?, status: String?, type: String?) {
        when (status) {
            ...
            "5" -> {
                val bundle = Bundle().apply {
                    putString("uri", type)
                    val localData=userDataLocal.userData?.get(position!!)
                    val data= localData?.recipe?.mealType?.get(0)?.split(",")
                    val formattedFoodName = data?.get(0)!!.replaceFirstChar { it.uppercase() }
                    putString("mealType", formattedFoodName)
                    putString("recipeID", localData.id.toString())
                    putString("serving", localData.servings.toString())
    ...
    override fun itemSelect(position: Int?, status: String?, type: String?) {
        ...
        storeUuid= position?.let { superMarketData[it].store_uuid.toString() }.toString()
        storeName= position?.let { superMarketData[it].store_name.toString() }.toString()
    }
