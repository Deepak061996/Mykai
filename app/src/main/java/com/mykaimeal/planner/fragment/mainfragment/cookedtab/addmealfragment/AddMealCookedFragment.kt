                    ...
                    is NetworkResult.Success -> {
                        try {
                            ...
                            if (searchModel.code == 200 && searchModel.success) {
                                searchModel.data?.let { it1 -> showDataInUi(it1) }?:run {
                                ...
                                if (searchModel.code == ErrorMessage.code) {
                                    ...
                                    if (!searchModel.message.equals("Search query cannot be empty.",true)) {
...
//        rcyData?.adapter = searchAdapterItem
    ...
    override fun itemClick(position: Int?, uri: String?, type: String?) {
        ...
        val formattedName = if (type.toString().equals("snack", ignoreCase = true)) {
            "Snacks"
        } else { type.toString().lowercase().replaceFirstChar { it.uppercaseChar() }
        }
