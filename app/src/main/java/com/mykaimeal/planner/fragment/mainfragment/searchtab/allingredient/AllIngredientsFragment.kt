...
class AllIngredientsFragment : Fragment(),View.OnClickListener,OnItemClickListener {
    ...
    private lateinit var binding: FragmentAllIngredientsBinding
...
//            adapterAllIngItem?.notifyDataSetChanged()
            binding.rcyAllIngredients.post {
    ...
    @SuppressLint("SetTextI18n")
    override fun itemClick(position: Int?, status: String?, selectType: String?) {
        if (selectType.equals("filter",true)){
            lastSelected=status.toString()
            ingredients.clear()
            binding.etIngRecipes.text.clear()
            pageReset()
