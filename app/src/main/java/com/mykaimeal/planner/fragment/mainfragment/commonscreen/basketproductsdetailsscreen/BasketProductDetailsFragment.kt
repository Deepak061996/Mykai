...
class BasketProductDetailsFragment : Fragment(), OnItemSelectListener {
    ...
    private fun showDataInUIFromBasket(data: BasketScreenModelData?) {
        try {
            ...
            result?.let {
                ...
                binding.tvProductName.text = BaseApplication.formatString(it.name.toString())
    ...
    private fun showDataInUI(data: MutableList<BasketProductsDetailsModelData>) {
       val dataList=data.distinctBy { it.name}
        dataList as MutableList
        basketScreenViewModel.setBasketProductDetail(dataList)
        basketProductsDetailsModelData.clear()
        dataList.let {
            basketProductsDetailsModelData.addAll(dataList)
        }
        resetLists()
    }
