    ...
    private fun initialize() {
        ...
        binding.rlGoToCheckout.setOnClickListener {
            if (binding.rlGoToCheckout.isClickable) {
                if (BaseApplication.isOnline(requireActivity())){
                    if (binding.textUpdate.text.toString().equals("0",true) || binding.textUpdate.text.toString().equals("0.0",true)) {
    ...
    private fun showDataInUI(data: BasketDetailsSuperMarketModelData?) {
        ...
        data?.store?.let {
            localStores.add(Store(null,null,null,it.store_name,
                it.store_uuid,storeImage,false,null,1,null,null))
        }
                ...
                if (type.equals("check",true)){
                    ...
                    apiModel.data?.let {
                        if (stores.isEmpty()){
                            ...
                            stores.sortByDescending { it.is_open?:false }
