...
class SettingViewModel @Inject constructor(private val repository: MainRepository) : ViewModel()  {
    ...
    suspend fun upDateProfileRequest(
        ...
        repository.upDateProfileRequestApi({ successCallback(it) },  name,bio,genderType,dob,height,heightType,activityLevel,heightProtein,calories,fat,carbs,protien,weight,weightType,targetWeight,targetWeightType,
    ...
    suspend fun updateDietSuggestionUrl(successCallback: (response: NetworkResult<String>) -> Unit,
        ...
        repository.updateDietSuggestionUrl({ successCallback(it) },gender,dob, height, heightType, weight, weightType, activityLevel,targetWeight,targetWeightType,goalDate,macros,calories,fatPer,proteinPer,carbsPer,typeStatus)
    }
